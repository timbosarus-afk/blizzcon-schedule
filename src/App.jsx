...
import { findClashes, formatDayLabel } from './utils/time';
...
function ScheduleApp() {
  ...
  return (
    <div>
      ...
      {view === 'schedule' && <HappeningNowSection events={events} />}

      <NextUpBanner events={events} favoriteIds={favoriteIds} />
      <NotifySetup status={push.status} subscribe={push.subscribe} />
      ...
      <div style={{ padding: '4px 16px 60px' }}>
        ...
        {visibleEvents.map((event, i) => {
          const showDayHeading = view === 'mine' && (i === 0 || visibleEvents[i - 1].day !== event.day);
          return (
            <div key={event.id}>
              {showDayHeading && <div className="day-heading">{formatDayLabel(event.day)}</div>}
              <EventCard
                event={event}
